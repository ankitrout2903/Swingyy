...
module.exports.register = async (req, res, next) => {
  try {
    ...
    // return res.json({ salt: salt, hashedPassword: hashedPassword });

    const insertQuery =
      "INSERT INTO user_table (email, username, password, uuid) VALUES (?, ?, ?, ?)";
    const values = [email, username, hashedPassword, uid];
    ...
    connection.query(insertQuery, values, (err, result) => {
      ...
      return res.json({
        status: true,
        user: { username, mail: email, password, _id: result.insertId },
